...
async function createSkill(page: Page, name: string, trigger: string) {
  const accessToken = await page.evaluate(() => localStorage.getItem('access_token'))
  if (!accessToken) {
    throw new Error('Missing access token after login')
  }

  const response = await page.request.post(`${API_BASE_URL}/api/v1/skills`, {
    headers: {
      Authorization: `Bearer ${accessToken}`,
      'Content-Type': 'application/json',
    },
  ...
  if (!response.ok()) {
    throw new Error(`Failed to create skill: ${response.status()} ${response.statusText()}`)
  }

  return (await response.json()) as { id: string }
}

async function deleteSkill(page: Page, skillId: string) {
  const accessToken = await page.evaluate(() => localStorage.getItem('access_token'))
  if (!accessToken) return

  await page.request.delete(`${API_BASE_URL}/api/v1/skills/${skillId}`, {
    headers: {
      Authorization: `Bearer ${accessToken}`,
    },
  })
...
test.describe('Chat input compact layout', () => {
  test('selected skills stay compact and do not squeeze the input area', async ({ page }) => {
    ...
    const skill = await createSkill(page, skillName, trigger)
